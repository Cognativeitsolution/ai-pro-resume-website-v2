import grammer from "media/services/image-1.png";
import tailored from "media/services/image-2.png";
import professional from "media/services/image-3.png";
import resume_parser from "media/services/image-4.png";

export const BannerData = {
  title: (
    <>
      Let's Perfect Your Resume
      <br className="hidden lg:block" /> & Cover Letter Together
    </>
  ),
  subtitle: "Craft Your Success Story :",
  description: (
    <>
      Writing a professional cover letter and resume can be challenging, and
      there's no guarantee they'll pass ATS checks. AI Pro Resume is here to help with
      expert resume and cover letter writing services. We also offer reviews to
      enhance your documents, providing tailored suggestions based on your job position
      and career goals.
    </>
  ),
};
// ==========================
export const ServicesData = [
  {
    image: grammer,
    title: "Professional Cover Letter Writing",
    features: [
      "Create a tailored cover letter for specific roles.",
      "Use persuasive language to highlight your value.",
      "Ensure a polished and professional tone.",
      "2 Revisions.",
    ],
    buttonText: "Add To Cart",
  },
  {
    image: tailored,
    title: "Professional Resume Review",
    features: [
      "Assess format, content, and ATS compatibility.",
      "Suggest best resume formats and improvements to display achievements.",
      "Our expert resume writers offer feedback to improve structure and clarity.",
      "2 Revisions",
    ],
    buttonText: "Add To Cart",
  },
  {
    image: professional,
    title: "Professional Resume Writing",
    features: [
      "Our professional resume writers design resumes that stand out.",
      "Modify content for ATS and role-specific requirements.",
      "Highlight skills and achievements with clear language.",
      "2 Revisions.",
    ],
    buttonText: "Add To Cart",
  },
  {
    image: resume_parser,
    title: "Professional Cover Letter Review",
    features: [
      "Highlight changes to make it concise and impactful.",
      "Suggest the best cover letter formats for job relevance and tone.",
      "Guide to align with industry standards.",
      "2 Revisions.",
    ],
    buttonText: "Add To Cart",
  },
];
// ==========================
export const ServicePageContent = [
  {
    title:
      "Steps To Get Our Professional Resume and Cover Letter Writing Service",
    description:
      "If you want to use our professional resume writing services and professional cover letter writing services, then follow the steps below:",
    features: [
      "Go to the home page of AI Pro Resume and log in to your account.",
      "Now, head to the service page present in the header of the website.",
      "You will notice the Resume and Cover Letter Writing Service on that page.",
      "Choose the service you want and click on add to cart.",
      "The checkout button appears next to it. Click on it.",
      "On the checkout page, you must upload your resume or cover letter.",
      "You will find the upload option beside the delete option.",
      "Once you are done with the file upload.",
      "Mark the checkbox and proceed to checkout.",
      "Once your resume or cover letter is ready, you will receive it via email within 2 days.",
      "You can also download your completed resume or cover letter directly from the dashboard.",
      "Go to the dashboard and click on My Services to access your file.",
    ],
  },
  {
    title: "Revision Process",
    description:
      "We offer two free revisions for each premium service. If you need updates to your document after delivery, follow these steps:",
    features: [
      "Log in to your account and navigate to the dashboard.",
      "You will see four headings: All Services, In Progress, Delivered, and Revisions.",
      "Click on Delivered and locate the Revision icon (a round arrow) next to your completed document.",
      "Click the icon to open a small text box.",
      "Enter the changes or updates you want to make and click Okay.",
      "Once the revised document is ready, it will appear under the Revisions section. You can download it directly from there.",
    ],
  },

  {
    title: "Steps to Get a Professional Resume or Cover Letter Review Services",
    description: "",
    features: [
      "Log in to your account and navigate to My Services from the header.",
      "Select the professional resume review or cover letter review service you want.",
      "Click on add to cart, and the checkout option will appear.",
      "Select the checkout button, and you will head to the checkout page.",
      "Upload the resume or cover letter you want our experts to review.",
      "Complete the payment process to confirm your order. You can purchase through coins and by buying the most popular or premium package.",
      "Within 2 days, you will receive an email with detailed suggestions and improvements for your resume or cover letter.",
      "You can also access the reviewed document from the dashboard under my services, where you will find a download option.",
    ],
  },
];
// ==========================
const ResumeExamplesPagefaqsList = [
  {
    question: "What Makes Us The Best Resume Maker In The Market?",
    answer:
      "Our unique features, such as AI-based suggestions, professional resume writing, professional cover letter writing, professional resume review, professional cover letter review, free resume and cover letter templates, pre-written examples, and referral rewards, make us the top and most creative resume builder.",
  },
  {
    question: "How Fast Can I Create My Resume?",
    answer:
      "You can create your resume in under 30 minutes using our ready-made templates and intuitive editor.",
  },
  {
    question: "Are The Resume Templates Customizable?",
    answer:
      "Yes, all our resume templates are fully customizable — fonts, layout, colors, and content.",
  },
  {
    question: "Do You Offer Cover Letter Services?",
    answer:
      "Yes, we offer cover letter writing and review services with multiple revisions for perfection.",
  },
];
export const faqs = {
  title: "FREQUENTLY ASKED QUESTIONS",
  faqlist: ResumeExamplesPagefaqsList,
};