...
export default function ProConsCard({ itemsList, description, title, icon, isPro, isTip }: ProsConsType) {
        ...
        <>
            <div className='relative pt-10 mt-5'>
                <div className='flex items-end gap-4 absolute -top-1 left-6  z-20'>
                    ...
                    <h3 className={`text-2xl capitalize font-bold ${isPro ? "text-green-600" : isTip ? "text-indigo-600" : "text-red-600"}`}>{title}</h3>

                </div>
                <div className='relative'>
                    <div className={`flex flex-col items-start gap-4 p-4 rounded-2xl shadow-sm w-[97%] sm:w-[98%] h-[98%] z-20 ${isPro ? "bg-green-100 border border-green-600" : isTip ? "bg-indigo-100 border border-indigo-600" : "bg-red-100 border border-red-600"} `}>
                        {description && <div className='mt-10'>
                            {Array.isArray(description) ? (
                                description?.map((item: any, index: number) => {
                                    return <p key={index} className="text-sm text-gray-800 p-2">{item}</p>;
                                })
                            ) :
                                <p className="text-sm text-gray-800 p-2">{description}</p>
                            }
                        </div>}

                        {itemsList?.length && <div className='mt-10'>
