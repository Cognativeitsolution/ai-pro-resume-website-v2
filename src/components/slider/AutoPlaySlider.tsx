...
export default function AutoPlaySlider(props: propsType) {
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel(
    { align: "start", dragFree: true, loop: true, ...options },
    ...
    <>
      ...
      {arrows && (
        <div className={`flex gap-10 justify-center mt-10 ${arrowPosition}`}>
          <PrevButton
            onClick={buttonHandlers.onPrevButtonClick}
            disabled={buttonHandlers.prevBtnDisabled}
          />
          <NextButton
            onClick={buttonHandlers.onNextButtonClick}
            disabled={buttonHandlers.nextBtnDisabled}
          />
        </div>
      )}
    </>
