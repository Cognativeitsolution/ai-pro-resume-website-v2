    ...
    <>
      <section className='py-5 md:py-10'>
        <div className='container'>
          <div className="w-full flex flex-col gap-[10px] justify-center items-center px-2 sm:px-6 py-8 bg-white/30 border-2 border-white rounded-2xl">
            <div className="mb-4 text-center">
              <h2 className="text-[26px] md:text-[30px] lg:text-[40px] leading-[36px] md:leading-[40px] lg:leading-[50px] font-semibold text-center mb-2">Buy Coins</h2>
              <p className="text-[16px] lg:text-[18px] font-normal text-center mt-5">
            ...
            <div className="flex flex-col items-center gap-4 container">
              <div className=" flex items-center justify-center flex-wrap gap-4 ">
                {coinPlans?.map((plan, index) => (
                  <div key={index} className={`cursor-pointer rounded-lg hover:bg-primary hover:text-white hover:font-medium border-2 shadow-[0px_0px_30px_rgb(220,220,230)] text-lg font-Lexend w-[120px] h-[100px] flex items-center justify-center relative flex-col ${selectedPlan?.id === plan.id ? "bg-PrimaryDark text-white" : "bg-white"}`} onClick={() => setSelectedPlan(plan)}>
                    {plan.discount > 0 && (
                      <div className={`${styles.discountbadge} text-white text-sm flex justify-center items-center absolute -top-[5px] -right-[5px]`}>
              ...
              {!isProceeding ? (
                <div className={`flex items-center justify-center w-full`}>
                  <button disabled={!selectedPlan} onClick={() => { setIsProceeding(true); }} className={`w-[200px] font-Lexend rounded-xl p-2 ${selectedPlan ? "bg-primary" : "bg-PrimaryDark"} text-white`} >
                    Proceed To Checkout
                  </button>
                </div>
              ) : (
                <div className={`rounded-2xl w-full xl:max-w-[90%] m-auto p-0 md:p-5`}>
              ...
              <div className="flex flex-col gap-5 mt-3">
                {HowToPurchaseCoins.map((item, index) => (
                  <div key={index}>
                    <h3 className="text-[26px] md:text-[30px] 2xl:text-[34px] leading-[36px] md:leading-[40px] lg:leading-[44px] font-semibold mb-2">
                      {item.title}
                    </h3>
                      ...
                      {item.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-4">
                          <Image
                            src={check_1}
                            alt="bullet"
                            className="mt-[7px] lg:mt-[5px] w-[10px] lg:w-[15px]"
                          />
                          <p className='text-[16px] font-normal'>{feature}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div>
                  <h3 className="text-[26px] md:text-[30px] 2xl:text-[34px] leading-[36px] md:leading-[40px] lg:leading-[44px] font-semibold mb-2">Referrals:</h3>
